...
const dateFormat = (timestamp) => {
	...
	let d = date.getDate()
	d = d < 10 ? "0" + d : d
	if (timestamp >= nowDayTimestamp) { // 当日
		let h = date.getHours()
		h = h < 10 ? "0" + h : h
		let i = date.getMinutes()
		i = i < 10 ? "0" + i : i
		return h + ":" + i
	} else if (timestamp < nowDayTimestamp && timestamp >= yesterday) { // 昨日
		return "昨日"
	} else if (timestamp < yesterday && timestamp >= nowYearTimestamp) { // 当年
		let m = date.getMonth() + 1
		m = m < 10 ? "0" + m : m
		let d = date.getDate()
		d = d < 10 ? "0" + d : d
...
export default {tabBarBadge, dateFormat, fileSizeCompute, longTimeFormat, getDistance, timeFormat}